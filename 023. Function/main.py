...
def sum(a, b):
    # 2개의 위치 인자(=필수 인자, 함수 호출 시 필수적으로 전달해야 함)
    return a + b
print(sum(1, 2))
# Positional하게 전달할 수도 있고
print(sum(b=3, a=2))
# 키워드 형태로 전달해줄 수 있다

def sum(a=0, b=0):
    # 기본값이 명시된 2개의 키워드 인자(=선택 인자, 함수 호출 시 선택적으로 전달할 수 있음)
    return a + b
print(sum(1, 2))
# 위치 인자처럼 순서대로 넘겨주거나
print(sum(b=3))
...
# 함수의 인자를 정의할 때,
# (1) 키워드 인자를 위치 인자 앞에 정의할 수 없고,
# (2) 가변 키워드 인자를 가변 위치 인자 앞에 정의할 수 없으며,
# (3) 가변 인자들을 위치 인자나 키워드 인자 앞에 정의할 수 없다

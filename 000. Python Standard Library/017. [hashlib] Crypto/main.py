...
# hashlib에서 사용할 수 있는 암호화 알고리즘은 아래처럼 확인할 수 있도
print(hashlib.algorithms_available)

# 해시는 암호학에서 널리 사용되지만 짧은 시간에 데이터를 검색하기 위해 설계되었기 때문에, brute-force에 취약하고(MD5는 보편적으로 1초 당 56억 개의 digest 대입 가능)
# 동일한 메시지가 언제나 동일한 digest를 갖는다면, 공격자가 전처리된 digest를 가능한 한 많이 확보한 다음 이를 탈취한 digest와 비교해 원본 메시지를 찾아낼 수 있음(rainbow attack)

# 왜 갑자기 클래스를 하냐고? staticmethod랑 classmethod란걸 알아냈으니까
class C:
    @staticmethod
    def static():
...
C.static()
C.cls()
# 일단 둘 다 인스턴스 없이 즉시 호출 가능하다

# 먼저 staticmethod는 평소 알고 있던 정적 메소드와 비슷하다
...
obj.cls()
# 클래스 메소드도 객체를 통해 호출이 된다. 도대체 무슨 차이일까 ㅡㅡ

# 정적 메소드는 호출된 클래스나 인스턴스에 대해 전혀 아무것도 모르는 메소드임
# 따라서 암시적인 첫번째 매개변수가 없음(self, cls)
...
# 결론 : 스태틱 메소드는 거의 쓸모 없고 클래스 메소드는 요긴할게 쓸 수 있다
